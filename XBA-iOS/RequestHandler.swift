...
class RequestHandler {
    ...
    static func getLatestNews(pageNumber: Int, completion: (data: [LatestNews]) -> Void){
        ...
        let url = self.fixEncodingUrl("\(Common.baseUrl)/latest/news?page=\(pageNumber)&key=\(Common.apiKey)")
    ...
    static func getNews(permalink: String, completion: (data: News) -> Void){
        let url = self.fixEncodingUrl("\(Common.baseUrl)/news?permalink=\(permalink)&key=\(Common.apiKey)")
    ...
    static func getLatestAchievements(pageNumber: Int, completion: (data: [LatestAchievements]) -> Void){
        let url = self.fixEncodingUrl("\(Common.baseUrl)/latest/achievements?page=\(pageNumber)&key=\(Common.apiKey)")
    ...
    static func getComments(permalink: String!, nID: String!, completion: (comments: [Comment]) -> Void){
        ...
        if (nID == nil) {
            url = self.fixEncodingUrl("\(Common.baseUrl)/comments?permalink=\(permalink)&key=\(Common.apiKey)")
        } else {
            url = self.fixEncodingUrl("\(Common.baseUrl)/comments?nID=\(nID)&key=\(Common.apiKey)")
        }
    ...
    static func getUpcomingGames(category: String, completion: (games: [UpcomingGame]) -> Void){
        let url = self.fixEncodingUrl("\(Common.baseUrl)/upcoming/games?category=\(category)&key=\(Common.apiKey)")
